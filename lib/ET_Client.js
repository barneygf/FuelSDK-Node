...
function _getSoapBaseUrl(client, origin, stack, soapOrigin, globalReqOptions, callback) {
...
var ET_Client = function(
    clientId,
    clientSecret,
    stack,
    apiVersion,
    tenant,
    { origin, authOrigin, soapOrigin, globalReqOptions = {} } = {}
) {
    if (!clientId || !clientSecret) {
        throw new Error('The client clientId and clientSecret props are required.');
    }

    if(apiVersion === 2 && !tenant) {
        throw new Error('ET_Client:: Version 2 APIs require that you specify a tenant');
    }

    if(tenant) {
        authOrigin = 'https://' + tenant + '.auth.marketingcloudapis.com/';
    }
	...
	_stack=stack;
	this.FuelAuthClient = this.initAuth(
        clientId,
        clientSecret,
        authOrigin,
        globalReqOptions,
...
ET_Client.prototype.initAuth = function(
    clientId,
    clientSecret,
    authOrigin,
    globalReqOptions,
    ...
    const authUrl = `${authOrigin}/v2/token`;
...
ET_Client.prototype.initSoap = function(auth, origin, stack, soapOrigin, globalReqOptions) {
    ...
    ['create', 'retrieve', 'update', 'delete'].forEach(method => { // for each method
        client[method] = function () {
            ...
            _getSoapBaseUrl(client, origin, stack, soapOrigin, globalReqOptions, () => { // get the custom base url
